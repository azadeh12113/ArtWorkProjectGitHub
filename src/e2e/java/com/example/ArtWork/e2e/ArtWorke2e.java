...
public class ArtWorke2e { 
  ...
  public void testCreateNewArtist_DisplayedInList() {
    ...
    String unique = "Azadeh-" + UUID.randomUUID().toString().substring(0, 8);

    driver.get(baseUrl + "/artists/new");
    driver.findElement(By.name("name")).sendKeys(unique);
    ...
    assertThat(table.getText()).contains(unique, "Painting", "2");
  ...
  public void testEditArtistScore_UpdatesRow() throws Exception {
    ...
    String unique = "Sara-" + UUID.randomUUID().toString().substring(0, 8);
    String id = postArtistJson(unique, "Photo", 1);
    ...
    assertThat(table.getText()).contains(unique, "Photo", "3");
